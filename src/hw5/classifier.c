    ...
    for(i = 0; i < m.rows; ++i){
        for(j = 0; j < m.cols; ++j){
            ...
            if (a == LOGISTIC){
                ...
                d.data[i][j] *= x > 0 ? 1.0 : 0;
            } else if (a == LRELU){
                d.data[i][j] *= x > 0 ? 1.0 : 0.1;
            }
        }
    }
}
...
void update_layer(layer *l, double rate, double momentum, double decay)
{
    ...
    matrix _0 = axpy_matrix(-rate, l->w, l->dw);
    matrix _1 = axpy_matrix(momentum, l->v, _0);
    free_matrix(l->v);
    l->v = _1;

    // Update l->w
    matrix new_w = axpy_matrix(rate, _1, l->w);
    free_matrix(l->w);
    l->w = new_w;
